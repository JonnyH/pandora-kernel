...
static inline int cifs_posix_convert_flags(unsigned int flags)
{
	int posix_flags = 0;
	...
	if (flags & O_APPEND)
		posix_flags |= (int)O_APPEND;
	if (flags & O_SYNC)
		posix_flags |= (int)O_SYNC;
	if (flags & O_DIRECTORY)
		posix_flags |= (int)O_DIRECTORY;
	if (flags & O_NOFOLLOW)
		posix_flags |= (int)O_NOFOLLOW;
	if (flags & O_DIRECT)
		posix_flags |= (int)O_DIRECT;
