...
#if defined(CONFIG_CS89x0)	// cs8900 is not use platform device format
	[0] = {
		.virtual	= (unsigned long)MDK_ETH_VIO_BASE,
		.pfn		= __phys_to_pfn(MDK_ETH_PIO_BASE),
		.length		= 0x10000,
		.type		= MT_DEVICE
	},
#elif defined(CONFIG_DM9000A)	// cs8900 is not use platform device format	
	[0] = {
		.virtual	= (unsigned long)MDK_ETH_VIO_BASE,
		.pfn		= __phys_to_pfn(MDK_ETH_PIO_BASE),
		.length		= 0x10000,
...
#ifdef CONFIG_MTD_AESOPPOLLUX
/* ghcstop, 080502: NOR Flash on aesop-pollux board */
static struct resource pollux_nor_resource[] = {
	[0] = {
		.start = POLLUX_PA_CS0,
		.end   = POLLUX_PA_CS0 + (CONFIG_MTD_AESOPPOLLUX_MAXSIZE*SZ_1M) - 1,
		.flags = IORESOURCE_MEM,
	}
};

static struct platform_device pollux_device_nor = {
	.name		= "pollux-nor",
	.id		= -1,
	.num_resources	= ARRAY_SIZE(pollux_nor_resource),
	.resource	= pollux_nor_resource,
};
#endif
