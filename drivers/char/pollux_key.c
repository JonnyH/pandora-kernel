...
unsigned char KeyTable[]={NON_KEY, VK_LEFT, VK_RIGHT, NON_KEY ,VK_UP, VK_UP_LEFT
											,VK_UP_RIGHT , VK_UP, VK_DOWN, VK_DOWN_LEFT ,VK_DOWN_RIGHT,
						  VK_DOWN, NON_KEY, VK_LEFT, VK_RIGHT, VK_TAT };				
...
ssize_t POLLUXkey_read(struct file *filp, char *Putbuf, size_t length, loff_t *f_pos)
{
	...
	unsigned short keyTemp=0xDFAA;
	
	if( ( (pollux_gpio_getpin(POLLUX_GPC11)) | 
					(pollux_gpio_getpin(POLLUX_GPC10)) ) == 0 )
	{
		keyTemp &= ~(1 << 8);				
	}else{
		keyTemp|=(pollux_gpio_getpin(POLLUX_GPC11) << 6);  //left
		keyTemp|=(pollux_gpio_getpin(POLLUX_GPC10) << 2);  //right
	}	
	
	keyTemp|=(pollux_gpio_getpin(POLLUX_GPC8) << 0); //up
	keyTemp|=(pollux_gpio_getpin(POLLUX_GPC9) << 4); //down
	
	keyTemp|=(pollux_gpio_getpin(POLLUX_GPC12) << 13);

	keyValue = (~keyTemp)& 0xFFFF;
        ...
        case IOCTL_GET_FW_VERSION:
            {
                ...
                fw_ver = FW_VERSION_WIZ;
#else
                fw_ver = FW_VERSION_VOCA;
#endif
                if( copy_to_user((unsigned char*)arg, &fw_ver, sizeof(unsigned int)) )
	                return -EFAULT;
            }
            return 0;
...
#ifndef CONFIG_ARCH_POLLUX_GPH_GBOARD
	for( i = 0 ; i < MAX_KEY ; i++){  
		pollux_set_gpio_func(BASE_KEY + i, POLLUX_GPIO_MODE_GPIO);
		pollux_gpio_set_inout(BASE_KEY + i, POLLUX_GPIO_INPUT_MODE);
	}
#else
