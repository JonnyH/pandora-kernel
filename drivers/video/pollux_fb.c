...
#else
static unsigned char board_num = BOARD_F300;
#endif	       
...
static int pollux_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
	...
	case FBIO_INFO:
		if(pdata)
		{
			...
			pOut->BlockMemPBase = POLLUX_MPEG2D3DBUFFER_PIO_BASE;
			pOut->BlockMemVBase = 0;	/* Not prepared yet */
			pOut->BlockMemSize  = (POLLUX_MPEG2D3DBUFFER_SIZE >> POLLUX_MPEG2D3D_BANK_SHIFT);
...
//				tpcol = m_ColorKey; // A status variable is needed
	...
	case  FBIO_GET_DIRTFLAG :
		if(pdata)
		{
			...
			result = ioctl_ops.get_dirtflag(layer, mlc_dev);
		}
		else
			result = -EFAULT;
		break;
