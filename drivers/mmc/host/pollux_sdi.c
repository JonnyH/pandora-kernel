...
static void pollux_sdi_request(struct mmc_host *mmc, struct mmc_request *mrq) 
{
    ...
    if(!ret) 
	{
		...
		if(host->complete_what == COMPLETION_WAIT_STOP)
		{
		    printk("Timeout occurred during waiting for the auto stop interrupt\n");
	...
	if(host->complete_what == COMPLETION_WAIT_STOP)
	{
	    printk(" mrq->cmd->resp[0] = 0x%x \n",mrq->cmd->resp[0]);
	    printk(" mrq->cmd->resp[1] = 0x%x \n",mrq->cmd->resp[1]);
	    printk(" mrq->cmd->resp[2] = 0x%x \n",mrq->cmd->resp[2]);
	    printk(" mrq->cmd->resp[3] = 0x%x \n",mrq->cmd->resp[3]);
	    
	    if(mrq->cmd->resp[1] & 0xFDF98008)
		{
		    host->mrq->data->error = MMC_ERR_TIMEOUT;
		}
	}
...
static void pollux_sdi_card_detect(struct work_struct *work)
{
	...
	{
	    mdelay(500);
	    if(pollux_gpio_getpin(host->cdpin)) {
	        host->cd_state = SD_REMOVED;
