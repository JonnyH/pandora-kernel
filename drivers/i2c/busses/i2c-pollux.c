...
static int pollux_i2c_wait_ack(struct pollux_i2c *i2c, CBOOL expect_ack)
{
	...
	if(!result) 
	{
		printk(" Error: Wait Ack Timeout %d\n", cnt);
		return CFALSE;
	}
	
	if(ack == CFALSE && expect_ack == CTRUE)
	{
		printk(" Error: Get NACK Response %d\n", cnt);
		return CFALSE;		
	}
	return CTRUE;
}
...
static int
pollux_i2c_read  (struct pollux_i2c *i2c)
{
	int ret = 0;
	CBOOL last   = CTRUE;
	struct i2c_msg	*msg;
...
static int
pollux_i2c_write  (struct pollux_i2c *i2c)
{
	int ret = 0;
	struct i2c_msg	*msg;
	U8 id = 0;
...
//	dump_i2c_registers(i2c);
	...
	// START condition & ID transmit
//	MES_I2C_SetAckGenerationEnable(MES_I2C0, CFALSE);									// ICCR: ACK_GEN-Not ACK, last Data
			...
			struct i2c_msg *msgs, int num)
{
	...
	for (i = 0; i < num; i++) {
		...
		if (cur_msg->len && msgs->buf) {	/* sanity check */
...
static int pollux_i2c_init(struct pollux_i2c *i2c)
{
	...
	i2c->clock_source = TXCLKSRC_PCLK256;   // ICCR    : SCL Source, ghcstop_caution, 추후에 clock scaler등은 손을 대야한다.
	i2c->clock_scale = 16;                 	// ICCR    : SCL SCALER 
	i2c->scl_sda_diff = 4;                  // QNT_MAX : SDA Delay Count

	MES_I2C_OpenModule(MES_I2C0);
	MES_I2C_SetClockPrescaler(MES_I2C0, i2c->clock_source , i2c->clock_scale);    // ICCR: SCL source: PCLK/256, SCL SCALER: 15 (12207 hz), scaler의 경우 안에서 -1이 된다.
	MES_I2C_SetDataDelay(MES_I2C0, i2c->scl_sda_diff);                            
	MES_I2C_SetInterruptEnableAll(MES_I2C0, CFALSE);
	MES_I2C_SetExtendedIRQEnable(MES_I2C0, CFALSE);
	MES_I2C_ClearInterruptPendingAll(MES_I2C0);
